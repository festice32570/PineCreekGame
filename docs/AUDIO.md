...
`snow_roll.wav` は連続ホワイトノイズと周期的なサイン波を廃止し、低域の非周期タイヤ振動に、圧雪がトレッドで砕ける短い帯域制限ノイズの粒を重ねた6秒ループです。

速度差は音量だけで表現し、`pitch_scale` は常に 1.0 です。旧方式のように走行速度で音源全体をピッチアップしないため、加速時に「音階が上がる」ようには聞こえません。
